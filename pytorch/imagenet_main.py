...
from models import imagenet_asl_resnet
from models import hgc_imagenet_asl_resnet
...
parser.add_argument('-a', '--arch', metavar='ARCH', default='asl')
...
parser.add_argument('--asl_lr', default=2e-2, type=float,
                    help='initial asl learning rate')
parser.add_argument('--momentum', default=0.9, type=float, metavar='M',
                    help='momentum')
parser.add_argument('--wd', '--weight-decay', default=1e-4, type=float,
                    metavar='W', help='weight decay (default: 1e-4)',
                    dest='weight_decay')
parser.add_argument('--asl_weight_decay', default=0, type=float)
...
#        model = models.__dict__[args.arch]()
    # create model
    if args.arch == 'asl':
        model = imagenet_asl_resnet(base_width=args.base_width)
    elif args.arch == 'hgc':
        model = hgc_imagenet_asl_resnet(base_width=args.base_width)
    else:
        NotImplementedError('No arch')
...
def train(train_loader, model, criterion, optimizer, epoch, args):
    ...
    info = {'train_loss': losses.avg, 'train_acc1': top1.avg} 
...
def validate(val_loader, model, criterion, args, epoch=0):
    ...
    # Logging in tensorboard
    info = {'val_loss': losses.avg, 'val_acc1': top1.avg} 
    for tag, value in info.items():
        logger.scalar_summary(tag, value, epoch+1)
