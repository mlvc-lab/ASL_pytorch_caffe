...
class BasicBlock(nn.Module):
    ...
    def __init__(self, in_planes, planes, stride=1):
        ...
        self.conv2 = nn.Conv2d(planes, planes, kernel_size=1, stride=stride, padding=0, groups=2, bias=False)
    ...
    def forward(self, x):
        ...
        N,C,H,W = out.size()
        out = out.view(N, 2, int(C/2), H, W).permute(0,2,1,3,4).contiguous().view(N,C,H,W) # shuffle
        out = self.conv2(self.asl(out))
        out += self.shortcut(x)
        return out
...
def shuffle_asl_resnet18(base_width, num_classes=10):
...
def test():
    net = shuffle_asl_resnet18(88).cuda()
