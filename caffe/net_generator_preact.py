...
#            top = [name]
    ...
    #************************** inplace **************************
    def ReLU(self, name=None, bottom=[]):
#        self.setup(self.suffix('relu', name), 'ReLU', inplace=True)
        self.setup(name, 'ReLU', bottom=bottom, inplace=True)
    
    def BatchNorm(self, name=None, bottom=[]):
#        self.setup(self.suffix('bn', name), 'BatchNorm', inplace=True)
        self.setup(name, 'BatchNorm', bottom=bottom, inplace=True)
    ...
    def Scale(self, name=None, bottom=[]):
#        self.setup(self.suffix('scale', name), 'Scale', inplace=True)
        self.setup(name, 'Scale', bottom=bottom, inplace=True)
...
#        self.conv_bn(name+'_conv1', num_output=num_output)
        self.BatchNorm(name + '_bn0', bottom=[bottom])
        self.Scale(name + '_scale0', bottom=[name + '_bn0'])
        self.ReLU(name + '_relu0', bottom=[name + '_scale0'])
        self.Convolution(name + '_conv0', bottom=[name + '_relu0'],
            num_output=num_input, kernel_size=3, pad=1, stride=1+int(up),
            group=num_input, bias=False)

        self.BatchNorm(name + '_bn1', bottom=[name + '_conv0'])
        self.Scale(name + '_scale1', bottom=[name + '_bn1'])
        self.ReLU(name + '_relu1', bottom=[name + '_scale1'])
        self.Convolution(name + '_conv1', bottom=[name + '_relu1'],
            num_output=num_output, kernel_size=1, pad=0, stride=1,
            group=1, bias=False)

        if up:
            self.Convolution(name + '_proj', bottom=[name + '_relu0'],
    ...
    def preact_resnet_cifar(self, n=3):
        """6n+2, n=3 9 18 coresponds to 20 56 110 layers
        first 3x3conv -> (ch 16, size 32)
        1st block = bn - relu - 1x1conv - bn - relu - 1x1conv -> (ch 16, size 32)
...
if __name__ == '__main__':
    ...
    pt_folder = "preact-resnet-n%d-w%d-shuffle" % (6*n+2, base_width)
    name = 'preact-resnet'+str(n)+'-cifar10'
