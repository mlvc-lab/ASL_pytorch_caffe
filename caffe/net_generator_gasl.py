...
#        self.BatchNorm(name + '_bn1', bottom=[name + '_conv0'], phase='TEST')
        ...
        multiplier = 9
        self.ActiveShift(name + '_asl', bottom=[name + '_conv0'], stride=1+int(up), multiplier=multiplier)

        # Slice
        self.Slice(name + '_slice', bottom=[name + '_asl'],
            top=[name + '_slice_' + str(i) for i in range(num_output)], 
            slice_points=[i*multiplier for i in range(1, num_output)])

        # Apply conv1x1 to each slice
        for i in range(num_output):
            self.Convolution(name + '_slice_conv_' + str(i), 
                bottom=[name + '_slice_' + str(i)],
                num_output=1, kernel_size=1, pad=0, stride=1, group=1, bias=False)

        # Concat 
        self.Concat(name + '_slice_concat',
            bottom=[name + '_slice_conv_' + str(i) for i in range(num_output)])

#        self.Convolution(name + '_conv1', bottom=[name + '_asl'],
#            num_output=num_output, kernel_size=1, pad=0, stride=1,
#            group=1, bias=False)
        self.Convolution(name + '_conv1', bottom=[name + '_slice_concat'],
            num_output=num_output, kernel_size=1, pad=0, stride=1,
            group=1, bias=False)
...
if __name__ == '__main__':
    ...
    solver = Solver(folder=args.name)
    solver.write()

    builder = Net(args.name)
    builder.ImageData('data_img/train.txt', phase='TRAIN', batch_size=32, shuffle=True, crop_size=32)
