...
class Net:
    ...
    def setup(self, name, layer_type, bottom=[], top=[], inplace=False):
        ...
        self.this_layer = new_layer
        if not inplace:
            self.cur_layer = new_layer

    def suffix(self, name, self_name=None):
        if self_name is None:
            return self.cur_layer.name + '_' + name
        else:
            return self_name
    ...
    def param(self, lr_mult=1, decay_mult=0):
        new_param = self.this_layer.param.add()
    ...
    def transform_param(self, 
        ...
        new_transform_param = self.this_layer.transform_param
        if scale != 1.:
    ...
    def data_param(self, source, backend='LMDB', batch_size=128):
        if backend == 'LMDB':
            new_data_param = self.this_layer.data_param
    ...
    def weight_filler(self, filler='msra'):
        """xavier"""
        if self.this_layer.type == 'InnerProduct':
            self.this_layer.inner_product_param.weight_filler.type = 'xavier' 
        else:
            self.this_layer.convolution_param.weight_filler.type = filler
    
    def bias_filler(self, filler='constant', value=0):
        if self.this_layer.type == 'InnerProduct':
            self.this_layer.inner_product_param.bias_filler.type = filler
            self.this_layer.inner_product_param.bias_filler.value = value
        else:
            self.this_layer.convolution_param.bias_filler.type = filler
            self.this_layer.convolution_param.bias_filler.value = value

    def include(self, phase='TRAIN'):
        if phase is not None:
            includes = self.this_layer.include.add()
            if phase == 'TRAIN':
...
#        self.setup(self.suffix('bn', name), 'BatchNorm', inplace=True)
        ...
        self.param(lr_mult=0, decay_mult=0)
        self.param(lr_mult=0, decay_mult=0)
        batch_norm_param = self.this_layer.batch_norm_param
...
#        self.setup(self.suffix('scale', name), 'Scale', inplace=)
        ...
        self.param(lr_mult=1, decay_mult=0)
        self.this_layer.scale_param.bias_term = True
...
#        self.image_data_param(source, batch_size=batch_size)
        new_image_data_param = self.this_layer.image_data_param
    ...
    def Concat(self, name=None, bottom=[]):
        self.setup(name, 'Concat', bottom=bottom)
        concat_param = self.this_layer.concat_param
        concat_param.axis = 1

    def Slice(self, name=None, bottom=[], top=[],
            axis=1, slice_points=[]):
        self.setup(name, 'Slice', bottom=bottom, top=top)
        slice_param = self.this_layer.slice_param
        slice_param.axis = axis
        slice_param.slice_point.extend(slice_points)

    def ActiveShift(self, name=None, bottom=[],
            pad=0, stride=1, multiplier=1,
            decay=False, freeze=False):
        self.setup(name, 'ActiveShift',  bottom=bottom)
        asl_param = self.this_layer.asl_param

        asl_param.pad = pad
        asl_param.stride = stride
        asl_param.multiplier = multiplier
    ...
    def ShuffleChannel(self, name=None, bottom=[], group=1):
        self.setup(name, 'ShuffleChannel',  bottom=bottom)
        shuffle_channel_param = self.this_layer.shuffle_channel_param
        shuffle_channel_param.group = group
    ...
    def Convolution(self, name, 
        ...
        conv_param = self.this_layer.convolution_param
...
#            num_output = self.bottom.convolution_param.num_output
    ...
    def SoftmaxWithLoss(self, name='loss', label='label'):
        self.setup(name, 'SoftmaxWithLoss', bottom=[self.cur_layer.name, label])

    def Softmax(self,bottom=[], name='softmax'):
        self.setup(name, 'Softmax', bottom=bottom)

    def Accuracy(self, name='Accuracy', label='label'):
        self.setup(name, 'Accuracy', bottom=[self.cur_layer.name, label])
    ...
    def InnerProduct(self, name='fc', num_output=10):
        ...
        self.param(lr_mult=1, decay_mult=0)    
        inner_product_param = self.this_layer.inner_product_param
    ...
    def Pooling(self, name, bottom=[], pool='AVE', global_pooling=False):
        ...
        if pool == 'AVE':
            self.this_layer.pooling_param.pool = self.this_layer.pooling_param.AVE
        else:
            NotImplementedError
        if global_pooling:
            self.this_layer.pooling_param.global_pooling = global_pooling
        else:
            self.this_layer.pooling_param.pad = 1
            self.this_layer.pooling_param.kernel_size = 3
            self.this_layer.pooling_param.stride = 2
    ...
    def Eltwise(self, name, bottom0, bottom1, operation='SUM'):
        ...
        if operation == 'SUM':
            self.this_layer.eltwise_param.operation = self.this_layer.eltwise_param.SUM
        else:
            NotImplementedError
    ...
    def res_func(self, name, num_input, num_output, first_group=1, second_group=1, up=False):
        bottom_name = self.cur_layer.name
        #print('name:', name)
        #print('bottom:', bottom)

        self.BatchNorm(name + '_bn0', bottom=[bottom_name], phase='TRAIN')
#        self.BatchNorm(name + '_bn0', bottom=[bottom_name], phase='TEST')
        ...
        self.Convolution(name + '_conv0', bottom=[name + '_bn0'],
            ...
            group=first_group, bias=False)

        #self.ShuffleChannel(name + '_shuffle', bottom=[name + '_conv0'], group=second_group)
...
#        self.BatchNorm(name + '_bn1', bottom=[name + '_conv0'], phase='TEST')
        ...
        self.ActiveShift(name + '_asl', bottom=[name + '_conv0'], stride=1+int(up), multiplier=1)

        self.Convolution(name + '_conv1', bottom=[name + '_asl'],
            num_output=num_output, kernel_size=1, pad=0, stride=1,
            group=second_group, bias=False)
        ...
        if up:
            ...
            self.Eltwise(name + '_sum', bottom0=bottom_name, bottom1=(name + '_conv1'))
    
    def res_block(self, group_id, n, num_input, num_output, first_group, second_group):
        def name(block_id):
        ...
        if group_id == 0:
            ...
            up = True
            temp_group = first_group

        self.res_func(name(0), num_input, num_output, first_group=temp_group, second_group=second_group, up=up)
        for i in range(1, n):
            self.res_func(name(i), num_output, num_output, first_group=temp_group, second_group=second_group)

    #************************** networks **************************
        
    def asl_resnet_cifar(self, n=3, base_width=16, first_group=1, second_group=1):
        ...
        for i in range(3):
            if i == 0:
                self.res_block(i, n, num_output, num_output*(2**i), first_group, second_group)
            else:
                self.res_block(i, n, num_output*(2**(i-1)), num_output*(2**i), first_group, second_group)
...
if __name__ == '__main__':
    ...
    builder.asl_resnet_cifar(n, args.bw, first_group, second_group)
